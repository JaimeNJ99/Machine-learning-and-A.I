...
#Maximum Likelihood Estimation (MLH) #
...
for i in range(num_agents):
    fitness[i] = MLH(np.array([agents[i]]))
    if i == 0:
...
while iter < max_iter:
    ...
    #New  values of tetha
    main_theta[0,0] = main_theta[0,0] - alpha * (MLH(tdelta0) - MLH(main_theta)) / delta
    main_theta[0,1] = main_theta[0,1] - alpha * (MLH(tdelta1) - MLH(main_theta)) / delta
    main_theta[0,2] = main_theta[0,2] - alpha * (MLH(tdelta2) - MLH(main_theta)) / delta
...
print("New data: ")
print("x1: 0, x2: 0")
classified = (best_position[0,0] + best_position[0,1] + best_position[0,2]) * (0 + 0)
classified = classifier(classified)
print("classified as: " + str(classified))
print("x1: 0, x2: 1")
classified = (best_position[0,0] + best_position[0,1] + best_position[0,2]) * (0 + 1)
classified = classifier(classified)
print("classified as: " + str(classified))
print("x1: 1, x2: 0")
classified = (best_position[0,0] + best_position[0,1] + best_position[0,2]) * (1 + 0)
classified = classifier(classified)
print("classified as: " + str(classified))
print("x1: 1, x2: 1")
classified = (best_position[0,0] + best_position[0,1] + best_position[0,2]) * (1 + 1)
classified = classifier(classified)
print("classified as: " + str(classified))
